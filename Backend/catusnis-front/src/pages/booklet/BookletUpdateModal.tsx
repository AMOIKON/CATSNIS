...
import { Booklet, BookletRequest, Region, District, Post, BookletStatus } from '../../types';
...
const BookletUpdateModal: React.FC<Props> = ({ show, onHide, onSuccess, booklet }) => {
    ...
    useEffect(() => {
        if (!show || !booklet) return;
        setForm({
            firstName: booklet.firstName,
            lastName:  booklet.lastName,
            contact:   booklet.contact,
            email:     booklet.email || '',
            region:   { id: booklet.region?.id   || 0 },
            district: { id: booklet.district?.id || 0 },
            post:     { id: booklet.post?.id      || 0 },
            status:   { id: booklet.status?.id    || 0 },
        });
        ...
        if (booklet.region?.id) {
            api.get(`/api/districts/region/${booklet.region.id}`)
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await BookletService.update(booklet.id, payload);
            onSuccess(); onHide();
        } catch (err: any) {
        ...
        <Modal show={show} onHide={onHide} centered size="lg">
                    ...
                    <div className="row g-3">
                        <div className="col-md-6">
                            <Form.Group>
                                ...
                                <Form.Control name="lastName" value={form.lastName} onChange={handleChange} className="rounded-3" />
                            </Form.Group>
                        </div>
                        <div className="col-md-6">
                            <Form.Group>
                                <Form.Label className="fw-semibold">Prénom <span className="text-danger">*</span></Form.Label>
                                <Form.Control name="firstName" value={form.firstName} onChange={handleChange} className="rounded-3" />
                            </Form.Group>
                        </div>
                        <div className="col-md-6">
                            <Form.Group>
                                <Form.Label className="fw-semibold">Contact <span className="text-danger">*</span></Form.Label>
                                <Form.Control name="contact" value={form.contact} onChange={handleChange} className="rounded-3" />
                            </Form.Group>
                        </div>
                        <div className="col-md-6">
                            <Form.Group>
                                <Form.Label className="fw-semibold">Email</Form.Label>
                                <Form.Control type="email" name="email" value={form.email} onChange={handleChange} className="rounded-3" />
                            </Form.Group>
                        </div>
                        <div className="col-md-6">
                            <Form.Group>
            ...
            <Modal.Footer className="border-0">
                ...
                <Button variant="warning" onClick={handleSubmit} disabled={isLoading} className="rounded-3 text-white">
                    {isLoading ? <><Spinner size="sm" className="me-2" />Modification...</> : <><i className="bi bi-pencil me-2" />Modifier</>}
                </Button>
            </Modal.Footer>
        </Modal>
    );
...
export default BookletUpdateModal;