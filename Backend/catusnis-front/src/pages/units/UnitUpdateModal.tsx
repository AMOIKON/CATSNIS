...
import { UnitsRequest, UnitsResponse } from '../../types';

interface Props {
    show:      boolean;
    onHide:    () => void;
    onSuccess: () => void;
    unit:      UnitsResponse | null;
}

const UnitUpdateModal: React.FC<Props> = ({ show, onHide, onSuccess, unit }) => {
    const [isLoading, setIsLoading] = useState(false);
    const [error,     setError]     = useState<string | null>(null);
    const [form,      setForm]      = useState<UnitsRequest>({ unitName: '' });

    useEffect(() => {
        if (show && unit) setForm({ unitName: unit.unitName });
    }, [show, unit]);
    ...
    const handleSubmit = async () => {
        ...
        setIsLoading(true);
        try {
            await UnitsService.update(unit.id, form);
            onSuccess();
            onHide();
        } catch (err: any) {
